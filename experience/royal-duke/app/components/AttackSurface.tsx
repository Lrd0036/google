...
export default function AttackSurface({ open, documentaryStage, endpoint, connection, rangeState, error, reportUrl, onClose, onRunAction, onReset, onApprove }: Props) {
  ...
  const divergenceSeconds = Math.min(THRESHOLDS.incidentContinuousSeconds, fleet?.divergence_elapsed_seconds ?? 0);
  const recoverySeconds = Math.min(THRESHOLDS.recoveryContinuousSeconds, fleet?.status === 'VERIFYING' && fleet.recovery_started_at ? Math.max(0, (Date.parse(fleet.updated_at ?? fleet.recovery_started_at) - Date.parse(fleet.recovery_started_at)) / 1000) : fleet?.recovery_elapsed_seconds ?? 0);
    ...
    <section className="surface-sheet cockpit-sheet" role="dialog" aria-labelledby="surface-title">
      ...
      <div className="cockpit-grid">
        ...
        <section className="cockpit-column evidence-column">
          ...
          <div className="pressure-stage"><div><span>{EXPERIENCE.process.operatorTelemetryLabel}</span><strong>{displayNumber(operator)} <small>{EXPERIENCE.process.pressureUnit}</small></strong><i>potentially compromised</i></div><div><span>{EXPERIENCE.process.independentTelemetryLabel}</span><strong>{displayNumber(physical)} <small>{EXPERIENCE.process.pressureUnit}</small></strong><i>trusted process telemetry</i></div><div><span>Delta</span><strong>{displayNumber(divergence)} <small>{EXPERIENCE.process.pressureUnit}</small></strong><i>&gt; {THRESHOLDS.incidentDeltaPsi} {EXPERIENCE.process.pressureUnit} required</i></div></div>
          <div className="timer-card"><div><span>Incident predicate</span><b>{divergenceSeconds.toFixed(1)} / {THRESHOLDS.incidentContinuousSeconds} sec</b></div><div className="timer-track"><i style={{ width: `${(divergenceSeconds / THRESHOLDS.incidentContinuousSeconds) * 100}%` }} /></div><p>The model does not decide when a process incident exists.</p></div>
          {(verifying || complete) && <div className="timer-card is-recovery"><div><span>Recovery verification</span><b>{complete ? THRESHOLDS.recoveryContinuousSeconds.toFixed(1) : recoverySeconds.toFixed(1)} / {THRESHOLDS.recoveryContinuousSeconds} sec</b></div><div className="timer-track"><i style={{ width: `${complete ? 100 : (recoverySeconds / THRESHOLDS.recoveryContinuousSeconds) * 100}%` }} /></div><p>Recovery is pressure above {THRESHOLDS.recoveryPressurePsi} {EXPERIENCE.process.pressureUnit} for {THRESHOLDS.recoveryContinuousSeconds} seconds—not whatever Gemini calls success.</p></div>}
          <div className="campaign-card"><div className="cockpit-section-head"><div><p className="surface-label">Campaign synthesis</p><h3>{campaign?.received ?? CAMPAIGN.received} events received</h3></div><span>seeded · reproducible</span></div><div className="campaign-funnel"><div style={{ '--funnel': '100%' } as CSSProperties}><b>{campaign?.routine ?? CAMPAIGN.routine}</b><span>routine / background</span></div><div style={{ '--funnel': '72%' } as CSSProperties}><b>{campaign?.decoys ?? CAMPAIGN.decoys}</b><span>decoys</span></div><div style={{ '--funnel': '52%' } as CSSProperties}><b>{campaign?.correlated_anomalies ?? CAMPAIGN.correlatedAnomalies}</b><span>correlated anomalies</span></div><div style={{ '--funnel': '36%' } as CSSProperties}><b>{campaign?.causal_events ?? CAMPAIGN.causalEvents}</b><span>causal events</span></div><div className="is-final" style={{ '--funnel': '24%' } as CSSProperties}><b>{campaign?.authoritative_facts ?? CAMPAIGN.authoritativeFacts}</b><span>authoritative facts</span></div></div><div className="fact-list">{facts.map((fact) => <details key={fact.fact_id} open={fact.status === 'PROVEN'}><summary><span className={fact.status === 'PROVEN' ? 'fact-proven' : ''}>{fact.status}</span>{fact.label}</summary><code>{fact.evidence_ids.join(' · ')}</code></details>)}</div></div>
        </section>
