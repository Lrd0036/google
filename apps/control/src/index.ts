...
import { MemoryApprovalReplayStore, verifyApprovalAssertion } from './authority.js';
...
const controller = new ExecutionController(new FirestoreExecutionStore(firestore));
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/executions') {
    try {
      const snapshot = await firestore.collection('executions').get();
  ...
  if (req.method === 'GET' && eventsMatch?.[1]) {
    try {
      ...
      const snapshot = await firestore.collection('executions').doc(executionId).collection('events').orderBy('sequence', 'asc').get();
  ...
  if (req.method === 'GET' && auditMatch?.[1]) {
    try {
      ...
      const executionSnapshot = await firestore.collection('executions').doc(executionId).get();
      ...
      const eventSnapshot = await firestore.collection('executions').doc(executionId).collection('events').orderBy('sequence', 'asc').get();
  ...
  if (req.method === 'GET' && executionMatch?.[1]) {
    try {
      const snapshot = await firestore.collection('executions').doc(routeExecutionId(executionMatch[1])).get();
  ...
  if (req.method === 'POST' && url.pathname === '/executions') {
    try {
      ...
      const runbook = (body.runbook ?? {}) as Record<string, unknown>;
      if (typeof body.tenant_id !== 'string' || !body.tenant_id || typeof runbook.id !== 'string' || typeof runbook.version !== 'number' || typeof runbook.ir_sha256 !== 'string' || typeof runbook.manifest_sha256 !== 'string' || typeof body.entry_node !== 'string') throw new Error('INVALID_EXECUTION_REQUEST');
      const document = body.document === undefined ? undefined : RBIRDocumentSchema.parse(body.document);
      if (document && (document.runbook.id !== runbook.id || document.runbook.version !== runbook.version || document.runbook.tenant_id !== body.tenant_id || document.capability_manifest.capability_manifest_sha256 !== runbook.manifest_sha256 || document.entry_node !== body.entry_node)) throw new Error('EXECUTION_DOCUMENT_CONTEXT_MISMATCH');
      const ref = firestore.collection('executions').doc(executionId);
      const execution = { execution_id: executionId, tenant_id: String(body.tenant_id ?? 'unknown'), status: 'PENDING', runbook: { id: String(runbook.id ?? 'unknown'), version: Number(runbook.version ?? 1), ir_sha256: String(runbook.ir_sha256 ?? `sha256:${'0'.repeat(64)}`), manifest_sha256: String(runbook.manifest_sha256 ?? `sha256:${'0'.repeat(64)}`) }, ...(document ? { runbook_document: document } : {}), cursor: { active_tokens: { main: { node_id: String(body.entry_node ?? 'unknown'), node_attempt: 1 } }, state_version: 1 }, control_epoch: 1, pending_approval: null, last_event_sequence: 0, last_event_hash: initialEventHash({ execution_id: executionId, tenant_id: String(body.tenant_id ?? 'unknown'), runbook: { id: String(runbook.id ?? 'unknown'), version: Number(runbook.version ?? 1), ir_sha256: String(runbook.ir_sha256 ?? `sha256:${'0'.repeat(64)}`), manifest_sha256: String(runbook.manifest_sha256 ?? `sha256:${'0'.repeat(64)}`) }, created_at: now }), context: body.context ?? {}, created_at: now, updated_at: now };
  ...
  if (localAuthorityEnabled && req.method === 'POST' && url.pathname === '/events/resume') {
    try {
      ...
      if (!input.assertion) throw new Error('SIGNED_APPROVAL_REQUIRED');
      {
        ...
        if (typeof asserted.sub !== 'string' || !localOperatorPrincipals.has(asserted.sub)) throw new Error('UNTRUSTED_LOCAL_OPERATOR');
        const assertion = verifyApprovalAssertion(input.assertion, localAuthorityKeys!.publicKey, {
          issuer: process.env.AUTHORITY_ISSUER || 'rb-authority-local',
